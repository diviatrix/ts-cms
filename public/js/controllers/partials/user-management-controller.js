import { AdminAPI, ProfileAPI } from '../api-core.js';
import { BaseAdminController } from '../pages/base-admin-controller.js';
import { renderCardTitle, renderMetaRow, renderEditButton } from '../components/ui-snippets.js';
...
export class UserManagement extends BaseAdminController {
    ...
    renderUsers(users) {
        if (users.length === 0) {
            this.elements.userListContainer.innerHTML = 'No users available yet.';
        } else {
    ...
    async loadUsers() {
        if (!AuthController.isAuthenticated()) {
            AuthController.handleAuthError({ status: 401 });
            return;
        }
        ...
        if (!response.success) { this.innerHTML = "Failed to load users"; }
    }
